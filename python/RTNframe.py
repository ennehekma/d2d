...
data = pd.read_csv(input_path_prefix + config["oscilating_elements"])





r = [data['x'][0],data['y'][0],data['z'][0]]
print "R = ", r
R = np.linalg.norm(r)
print R
U_R = (r/R)
...
v = [data['xdot'][0],data['ydot'][0],data['zdot'][0]]
print "V = ", v
V = np.linalg.norm(v)
h = np.cross(r,v)
H = np.linalg.norm(h)
...
print "ut cros ur", np.cross(U_T,U_R)

gamma = np.matrix([U_R,U_T,U_N])
# print type(gamma)
print gamma

atom_dep_napa1 = np.transpose([-206.0873238391, -118.4009287948, -50.2024520553]) 
# mag = np.linalg.norm(atom_dep_napa1)
lambert_dep_napa1 = np.transpose([-190.92792851,   -117.8833454456, -46.0119028882]) 
atom_arrival_napa1 = np.transpose([54.2533981172,    -31.3222647748,  -97.3365674194]) 
lambert_arrival_napa1 = np.transpose([43.3202050547, -29.7451833227,  -97.0459927329]) 
atom_dep_mag = np.linalg.norm(atom_dep_napa1)
lambert_dep_mag = np.linalg.norm(lambert_dep_napa1)
atom_arrival_mag = np.linalg.norm(atom_arrival_napa1)
lambert_arrival_mag = np.linalg.norm(lambert_arrival_napa1)


atom_dep_napa1 = np.transpose([10,20,30]) 

# print atom_dep_napa1

atom_dep = np.inner(gamma,atom_dep_napa1)
lambert_dep = np.inner(gamma,lambert_dep_napa1)
atom_arrival = np.inner(gamma,atom_arrival_napa1)
lambert_arrival = np.inner(gamma,lambert_arrival_napa1)

atom_dep_norm = np.linalg.norm(atom_dep)
lambert_dep_norm = np.linalg.norm =(lambert_dep )
atom_arrival_norm = np.linalg.norm =(atom_arrival )
lambert_arrival_norm = np.linalg.norm =(lambert_arrival )



print "atom_dep = ",atom_dep, atom_dep_norm, atom_dep_mag
print "lambert_dep = ",lambert_dep, lambert_dep_norm, lambert_dep_mag
print "atom_arrival = ",atom_arrival, atom_arrival_norm, atom_arrival_mag
print "lambert_arrival = ",lambert_arrival, lambert_arrival_norm, lambert_arrival_mag
