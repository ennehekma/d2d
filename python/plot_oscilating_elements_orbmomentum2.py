...
plt.plot(oscilating_elements['jd'],np.sqrt(oscilating_elements['a']*(1-oscilating_elements['e']*oscilating_elements['e'])*398600),linewidth=2,label='Atom')
plt.plot(oscilating_elements['jd'],np.sqrt(oscilating_elements['La']*(1-oscilating_elements['Le']*oscilating_elements['Le'])*398600),label='Lambert',linewidth=2)
ax1.xaxis.set_major_formatter(formatter)
ax1.yaxis.set_major_formatter(formatter)
plt.xlabel('Time since beginning of the transfer [s]')
plt.ylabel('Magnitude of specific angular momentum vector [km$^2$/s]')
# plt.title('Osculating element of transfer')
plt.legend(loc='best',prop={'size':9})
plt.grid()
plt.savefig(output_path_prefix + "angular_momentum_keplerian_" + config["filename"], dpi=config["figure_dpi"])
plt.clf()
