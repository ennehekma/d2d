...
for i in range(10,90,10):
    ...
    # for x in xrange(10,12):
        ...
        # for y in xrange(1,2):
            ...
            # print newnew
            plt.step(newnew['generation'],newnew['transfer_delta_v'],c=c,where='post',lw=.1)
        plt.title(str(departure_object_id) + " to " + str(arrival_object_id))

        plt.axis([0,generations,0,1])
        plt.tight_layout()

        plt.savefig("/home/enne/work/d2d/data/pagmo/figures/"+ str(departure_object_id) +"_"+str(arrival_object_id)+"_pop_" +str(i)+".png", dpi=300)
        plt.close()
