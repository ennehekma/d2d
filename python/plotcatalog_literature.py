...
outputPath 					= "../data/plots_literature/"
...
plt.plot(ecc3['HEO']*np.cos(aop3['HEO']),ecc3['HEO']*np.sin(aop3['HEO']), 						  \
		 	marker='D', markersize=6, color='r', linestyle='none')
plt.axis('equal')
axis.set_xlim(xmin=-.021, xmax=.021)
axis.set_ylim(ymin=-.021, ymax=.021)
axis.set(xticks=[-.02,-.01,0,.01,.02])
axis.set(yticks=[-.02,-.01,0,.01,.02])
figure.set_tight_layout(True)
