...
# for i in xrange(0,departure_epochs.size):
    ...
    scan_data = pd.read_sql("SELECT departure_object_id, arrival_object_id,                       \
                                    min(transfer_delta_v), departure_argument_of_periapsis,       \
                                    departure_longitude_of_ascending_node,                        \
                                    departure_true_anomaly, arrival_position_x\
                                FROM lambert_scanner_results                                      \
    ...
    scan_data.columns = ['departure_object_id','arrival_object_id',                               \
                         'transfer_delta_v','departure_argument_of_periapsis',                    \
                         'departure_longitude_of_ascending_node',                                 \
                         'departure_true_anomaly','longitude']
    print scan_data
    print scan_data['departure_argument_of_periapsis'][0]
    print len(scan_data)
    for x in xrange(0,int(len(scan_data))):
        scan_data['longitude'][x] =  (scan_data['departure_argument_of_periapsis'][x] + scan_data['departure_true_anomaly'][x] + scan_data['departure_longitude_of_ascending_node'][x])%(2*np.pi)
                                               
                                           
    print scan_data
    scan_order = scan_data.sort_values('longitude')                                            \
                          .drop_duplicates('departure_object_id')[                                \
                              ['departure_object_id','longitude']]
    ...
    ax2.step(np.arange(0.5,scan_map.shape[1]+.5),scan_order['longitude'],'k',linewidth=2.0)
